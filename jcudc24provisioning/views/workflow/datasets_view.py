...
class DatasetsView(Workflows):
    ...
    def __init__(self, request):
        ...
        self.form = Form(self.schema, action="submit_datasets", buttons=('Save', 'Delete'), use_ajax=False)

    @view_config(renderer="../../templates/form.pt", name="submit_datasets")   # Use ../../templates/submit.pt for AJAX - File upload doesn't work due to jquery/deform limitations
    def submit(self):
...
#            print "appstruct: " + str(appstruct)
        ...
        return {"page_title": 'Datasets', "form": self.form.render(appstruct)}

    @view_config(renderer="../../templates/form.pt", name="datasets")
    def datasets_view(self):
        return {"page_title": 'Datasets', "form": self.form.render()}
