...
class SetupSchema(Base):
    __tablename__ = 'project'

    id = Column(Integer, primary_key=True, nullable=False, ca_widget=deform.widget.HiddenWidget())

    project_title = Column(String(), ca_widget=deform.widget.TextInputWidget(css_class="full_width"),
    ...
    brief_description = Column(String(),
        ...
        ca_widget=deform.widget.TextAreaWidget(rows=3), ca_title="Brief Description",
    ...
    full_description = Column(String(), ca_widget=deform.widget.TextAreaWidget(rows=6),
    ...
    data_manager = Column(String(), ca_title="Data Manager",
    ...
    project_lead = Column(String(), ca_title="Project Lead",
        ca_widget=deform.widget.AutocompleteInputWidget(min_length=1, values=choices),
        ca_placeholder="Autocomplete from JCU's accounts (press the 'Other' tick box for non-JCU)",
